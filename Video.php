...
class Video {
    ...
    public function exp($turl){
        $url = $this->regxGetUrl($turl);
        $ret = [];
        if(stripos($url,'douyin.com') !== false){
            $ret = $this->douyin($url);
        }else if(stripos($url,'ixigua.com') !== false){
            $ret = $this->xigua($url);
        }else if(stripos($url,'kuaishou.com') !== false){
            $ret = $this->kuaishou($url);
        }else if(stripos($url,'immomo.com') !== false){
            $ret = $this->momo($url);
        }else if(stripos($url,'meipai.com') !== false){
            $ret = $this->meipai($url);  // 有水印
        }else if(stripos($url,'pearvideo.com') !== false){
            $ret = $this->pear($url);
        }else if(stripos($url,'weibo.com') !== false){
            $ret = $this->weibo($url);
        }else if(stripos($url,'pipix.com') !== false){
            $ret = $this->pipixia($url);
        }else if(stripos($url,'xiaochuankeji.cn') !== false){
            $ret = $this->zuiyou($url);
        }else if(stripos($url,'kg3.qq.com') !== false){
            $ret = $this->quanminkge($url);
        }else if(stripos($url,'huoshan.com') !== false){
            $ret = $this->huoshan($url);
        }else if(stripos($url,'xinpianchang.com') !== false){
            $ret = $this->xinpianchang($url);
        }else if(stripos($url,'pipigx.com') !== false){
            $ret = $this->pipigaoxiao($url);
        }else{
            $ret = $this->third($url);
        }
        return $ret;
     ...
     * @return void
     */
    private function regxGetUrl($txt){
        $rs = preg_match_all('#\bhttps?://[^,\s()<>]+(?:\([\w\d]+\)|([^,[:punct:]\s]|/))#', $txt, $match);
        if($rs && isset($match[0][0])){
            return $match[0][0];
        }
        return false;
    }
    ...
    /**
     * 第三方解析。
     * 需要购买：videoparse.cn，自行决定。
     * @param $url
     * @return void
     */
    private function third($url){
        $furl = 'https://api-sv.videoparse.cn/api/mini/parse?appid=xxxx&appsecret=xxx&url='.urlencode($url);
        $json = $this->curl_get($furl);
        if($json){
    ...
    public function weishi($url) {
        //https://isee.weishi.qq.com/ws/app-pages/share/index.html?wxplay=1&id=7kYbWkiZY1OeHIxYw&spid=3339227596592716038&qua=v1_iph_weishi_8.75.1_131_app_a&chid=100081014&pkg=3670&attach=cp_reserves3_1000370011
    ...
    private function weibo($url) {
        //https://m.weibo.cn/status/4787967690806128
    ...
    private function xhs($url){
        ...
        //$turl = 'https://www.xiaohongshu.com/web-login/canvas?redirectPath='.urlencode($loc);
    ...
    private function xiaokaxiu($url) {
        preg_match('/id=(.*)\b/', $url, $id);
    ...
    private function pipigaoxiao($url) {
    ...
    private function quanminkge($url) {
        preg_match('/\?s=(.*)/', $url, $id);
        $text = $this->curl_get('https://kg.qq.com/node/play?s=' . $id[1]);
     ...
     * @param $url
     */
    private function curl_get($url,$gzip=false,$firefox=false) {
        if($firefox) {
            //火狐浏览器
    ...
    //添加请求头
    private function FormatHeader($url,$useragent) {
        // 解析url
        $temp = parse_url($url);
    ...
    private function xigua($url) {
        ...
        $videos = $result["videoResource"]["normal"]["video_list"];
        $url = isset($videos['video_4']) ? $videos["video_4"]["main_url"] : $videos['video_2']['main_url'];
        $video_url = base64_decode($url);
        //var_dump($result["videoResource"]["dash"]);
...
//                        ]
    ...
    private function xinpianchang($url,$i=0) {
        ...
        if(!isset($vid[1]) || !isset($key[1])){
            if($i<3){
                usleep(500000);
                $res = $this->xinpianchang($url,++$i);
                if($res){
                    return $res;
                }
            }else{
